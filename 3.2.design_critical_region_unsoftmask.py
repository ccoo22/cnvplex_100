...
input_dir = "./noney/critical_region/"
input_file = os.path.join(input_dir, "critical_region_design.txt")  # 探针设计文件
unsoftmask_file = './target.txt.nocnv'  # 非小写字母覆盖区域

output_file = os.path.join(input_dir, "critical_region_design.unsoftmask.txt")
...
# 每个步长的数据计算overlap
for step in ['cnvplex_1K', 'cnvplex_10K', 'cnvplex_100K', 'cnvplex_200K']:
    print("处理步长：" + str(step))
